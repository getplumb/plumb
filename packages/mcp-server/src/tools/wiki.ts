...
const TRAFFIC_TELEMETRY_DIR = join(homedir(), '.plumb', 'telemetry');
...
export function registerWikiQueueEdit(server: McpServer, config: WikiToolsConfig): void {
  server.tool(
    ...
    'Queue a wiki edit request for async processing. Immediately appends the fact to the wiki edit queue and returns — no latency added to your response. The background worker integrates the fact into the relevant wiki page(s) within 60 seconds.',
    {
      fact: z
        .string()
        .describe('The fact or update to incorporate into the wiki, written in plain English. Include enough context for the wiki writer to place it correctly (e.g. "Jordan Lee left Northwind as of April 2026"). Maximum 10,000 characters after trimming.'),
    },
    async (args) => {
      try {
