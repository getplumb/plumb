...
async function seedWikiDb(dbPath: string, wikiRoot: string): Promise<void> {
  ...
  await writePage(wikiRoot, 'people/alex.md', '# Alex\n\nLinks to [[Plumb V2]].');
  ...
  for (const row of [
    ...
    ['people/alex', 'people/alex.md', 'person', 'Alex', hashContent(await readFile(join(wikiRoot, 'people/alex.md'), 'utf8'))],
  ]) {
  ...
  for (const row of [
    ['people/alex', 'Plumb V2', 'projects/plumb-v2', 1],
    ['projects/plumb-v2', 'Alex', 'people/alex', 1],
  ]) {
    const stmt = db.prepare(
...
describe('MCP server tool schemas and responses', () => {
  ...
  describe('wiki tools', () => {
    ...
    it('returns inbound and outbound links from wiki.db', async () => {
      ...
      expect(result.content[0]!.text).toContain('[[Alex]] → people/alex');
      expect(result.content[0]!.text).toContain('← people/alex via [[Plumb V2]]');
    });
