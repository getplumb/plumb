...
Engineering Manager at Northwind. Based in San Francisco.

## Background

Jordan joined Northwind in 2022.
`;
...
describe('parsePatchOp', () => {
  ...
  test('parses replace_paragraph op', () => {
    const op = parsePatchOp({
      ...
      anchor: 'joined Northwind',
      replacement: 'Jordan joined Northwind in 2021 as Senior EM.',
    });
  ...
  test('parses update_line op', () => {
    const op = parsePatchOp({
      ...
      replacement: 'Engineering Manager at Northwind (promoted 2024).',
    });
...
describe('tombstone', () => {
  ...
  test('hashSentence is deterministic', () => {
    const h1 = hashSentence('Jordan joined Northwind in 2022.');
    const h2 = hashSentence('Jordan joined Northwind in 2022.');
    assert.equal(h1, h2);
  });

  test('normalizeSentence trims, collapses whitespace, and lowercases', () => {
    const n = normalizeSentence('  Jordan joined  Northwind. ');
    assert.equal(n, 'jordan joined northwind.');
  });

  test('addTombstone + isTombstoned: matches exact sentence', () => {
    addTombstone(db, 'Jordan joined Northwind in 2022.');
    const result = isTombstoned(db, 'Jordan joined Northwind in 2022.');
...
describe('applier body ops', () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(
      join(tempDir, 'companies', 'northwind.md'),
  ...
  test('replace_paragraph op modifies body', async () => {
    ...
    const result = await applyPatch(
      {
        ...
        ops: [
          {
            op: 'replace_paragraph',
            anchor: 'joined Northwind in 2022',
            replacement: 'Jordan joined Northwind in 2021 as a Senior Engineering Manager.',
          },
    ...
    assert.ok(!page.body.includes('joined Northwind in 2022'));
    ...
    const result = await applyPatch(
      {
        ...
        ops: [
          {
            op: 'update_line',
            anchor: 'Engineering Manager at Northwind',
            replacement: 'Engineering Manager at Northwind. Promoted to Director in 2025.',
          },
          { op: 'set_frontmatter', key: 'updated', value: '2026-04-16' },
        ],
      },
...
describe('worker — end-to-end coalescing (Jordan Lee)', () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(
      join(wikiRoot, 'companies', 'northwind.md'),
      SAMSARA_FM_YAML + SAMSARA_BODY,
    );
  });
  ...
  test('5 Jordan Lee facts coalesce to exactly 1 write per page', async () => {
    ...
    const facts = [
      'Jordan Lee was promoted to Director of Engineering at Northwind in March 2025.',
      'Jordan Lee leads the platform team at Northwind with 12 engineers.',
      'Jordan Lee is based in San Francisco, CA.',
      'Jordan Lee graduated from UC Berkeley in 2015 with a BS in CS.',
      'Jordan Lee joined Northwind from Lyft where he was a Staff Engineer.',
    ];
    ...
    const mockResolver = async (
      ...
      return ['people/jordan-lee.md', 'companies/northwind.md'];
    };
    ...
    const northwindPath = join(wikiRoot, 'companies', 'northwind.md');
    ...
    const northwindContent = await readFile(northwindPath, 'utf8');
    ...
    assert.ok(
      northwindContent.includes('Recent Updates'),
      'northwind.md should have been updated',
    );
