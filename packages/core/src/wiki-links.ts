 ...
 * it — silently dropped every link written as a path (`[[people/priya]]`),
 * with a `.md` suffix, or by display name against a kebab-case filename
 * (`[[Morgan Ellis]]` → `people/morgan-ellis.md`, 23 live occurrences).
...
export function syncWikiLinks(
   ...
   * cross-references on this wiki actually match on — `[[the user]]`,
 ...
 * change for some unrelated reason. Live instance: creating `companies/halcyon.md`
 * left `people/robin-vance.md`'s `[[Halcyon]]` row unresolved. A full
 ...
 *      `[[people/priya]]` (path), `[[Priya.md]]` (extension),
 *      `[[Morgan Ellis]]` (display name against a kebab-case stem) or
 *      `[[the user]]` (frontmatter alias) all fail that comparison. Wiring the old
