 ...
 * prose, not in a schema, so nothing ever measured it. the user suspected this was
 * wrong before anyone looked; he was right, and the location of the bug was the
 * prompt.
 *
 * The nightly dream found the concrete failure on 2026-08-14: Priya's
 * Meridian Grid title was stale on BOTH `people/priya.md` and
 * `companies/meridian-grid.md`, and the dream could do nothing but send an email
 * about it. `companies/meridian-grid.md:105` carries the sentence "[[Joe]],
 * [[Nadia]], and [[Rosa]] were true peers of the user's, alongside [[Priya]]
 * (VP of Customer Success, while the user was Head of Product Management)" — a title
 * claim about Priya living on a page that is not Priya's.
 *
 * That is not an accident of this one fact. It follows from how the wiki has to
 * work: chunk-level retrieval means a page that only says "see [[Priya]]"
 ...
 * wiki 2026-08-14, `people/priya.md` has 10 inbound pages, and
 * `companies/meridian-grid.md` — the exact page the dream flagged — is one of them.
 *
 ...
 * files pages — `people/morgan-ellis.md` is linked as `[[Morgan Ellis]]`
...
export function collectFanoutCandidates(options: FanoutOptions): FanoutResult {
  ...
  // stores `people/priya.md`, the resolver's canonical form is
  // `people/priya`. Normalizing BOTH sides here rather than asking callers to
