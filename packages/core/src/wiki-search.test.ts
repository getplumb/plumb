...
describe('WikiSearch.search — basic', () => {
  ...
  test('active contextual sqft rescue replaces representative child without changing page score or ordering', async () => {
    ...
    await seedPage(dbPath, {
      ...
      chunks: [
        { content: 'sharedneedle sharedneedle sharedneedle overview winner for Clay home.', section: 'Overview' },
        { content: 'Property evidence: Clay home is exactly 2,112 square feet with a finished basement.', section: 'Property Details' },
      ],
    });
    await seedPage(dbPath, {
      pageId: 'other/filler',
    ...
    const searcher = await WikiSearch.create({ wikiRoot, dbPath, preCheck: false, contextualRetrieval: { mode: 'active' } });
    try {
      const baseline = await searcher.search('sharedneedle Clay home', 5);
      const rescued = await searcher.search('sharedneedle Clay home square footage', 5);
      ...
      assert.ok(!rescued[0]!.parentContext?.includes('overview winner for Clay home'));
