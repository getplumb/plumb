 ...
 *
 * EVERY CASE IS A REAL INCIDENT from Alex's wiki, reconstructed as the smallest
...
// field-guide deliverable Alex is sending a friend...") produced a page with
...
A field-guide deliverable Alex is sending a friend who is setting up a company
knowledge base.
`;
  ...
  it('passes when the same page is written correctly', async () => {
    ...
    writeFiles(root, {
      'projects/company-wiki-brief.md': page({
        ...
        body: 'A field-guide deliverable Alex is sending a friend setting up a company knowledge base. Related: [[Plumb 2.0]].',
      }),
    });
...
describe('pre-existing findings on unrelated pages', () => {
  it('cancel by subtraction and do not fail a good edit', async () => {
    const root = makeWiki({
      ...
      'people/alex-rivera.md': page({ type: 'person', title: 'Alex Rivera', body: 'Product lead.' }),
    });
    ...
    writeFiles(root, {
      'people/alex-rivera.md': page({
        type: 'person',
        title: 'Alex Rivera',
        body: 'Product lead. Currently interviewing at Halcyon for Head of Product.',
      }),
    });
...
describe('an edit that repairs an existing finding', () => {
  it('passes and reports the resolved finding', async () => {
    const root = makeWiki({
      'people/robin-vance.md': page({
        type: 'person',
        title: 'Robin Vance',
        body: 'Director of Global Talent Acquisition at [[Halcyon]].',
      }),
    });
    const before = await snapshotWikiStructure(root);
    expect(before.totals.unresolved).toBe(1);
    ...
    writeFiles(root, {
      'companies/halcyon.md': page({ type: 'company', title: 'Halcyon', body: 'Acquired Locusview.' }),
    });

    const verdict = verifyWikiStructure(before, await snapshotWikiStructure(root));
    expect(verdict.ok).toBe(true);
    expect(verdict.resolvedFindings).toHaveLength(1);
    expect(verdict.resolvedFindings[0]?.page).toBe('people/robin-vance.md');
  });
});
...
describe('a page filed outside the schema directories (queue item 8b422737, 2026-08-14)', () => {
  // The real fact ended: "File: memory/docs/2026-08-04-latchkey-sdk-jd.md" — a
  ...
  it('is a finding even when the page is otherwise schema-clean', () => {
    const before = makeWiki({
      'companies/latchkey.md': page({ type: 'company', title: 'Latchkey' }),
    });
    const after = makeWiki({
      ...
      'memory/docs/2026-08-04-latchkey-sdk-jd.md': page({
        type: 'project',
        title: 'Latchkey SDK Job Description',
      }),
    });
    ...
    return (async () => {
      ...
      expect(placement[0]!.page).toBe('memory/docs/2026-08-04-latchkey-sdk-jd.md');
  ...
  it('does not fire on any directory the live wiki actually uses', async () => {
    const root = makeWiki({
      'people/alex-rivera.md': page({ type: 'person', title: 'Alex Rivera' }),
      'companies/latchkey.md': page({ type: 'company', title: 'Latchkey' }),
      'tools/plumb.md': page({ type: 'tool', title: 'Plumb' }),
      'projects/plumb-20.md': page({ type: 'project', title: 'Plumb 2.0' }),
      'interviews/northwind-loop.md': page({ type: 'interview', title: 'Northwind Loop' }),
