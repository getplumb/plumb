 ...
 *
 * The threshold cases are built from the values measured on Alex's live wiki on
...
function liveReport(): Omit<WikiIntegrityReport, 'breaches' | 'ok'> {
  return {
    ...
    dbPath: join(homedir(), '.plumb', 'wiki.db'),
    links: {
      unresolved: 2,
      ambiguous: 0,
      anchorMissing: 0,
...
describe('evaluateIntegrity', () => {
  ...
  test('stale index rows breach, and the message names the pages', () => {
    ...
    const breaches = evaluateIntegrity({
      ...r,
      index: { ...r.index, stale: ['people/priya.md', 'projects/company-wiki-brief.md'] },
    });
    assert.equal(breaches[0]?.check, 'index.stale');
    assert.match(breaches[0]?.message ?? '', /people\/priya\.md/);
  });
...
describe('frontmatterKeysPresent', () => {
  /**
   * people/priya.md as it stands on the live wiki. Its `confidence: high`
  ...
  const KARTHIK = [
    ...
    'tags: [former-manager, calibrix, reference]',
    'confidence: high',
    "summary: Alex's manager at Calibrix.",
    '---',
    '',
    '# Priya',
  ].join('\n');
...
describe('isGeneratedWikiPage', () => {
  ...
  test('real content pages are not generated, including in subdirectories', () => {
    for (const p of ['people/priya.md', 'companies/halcyon.md', 'projects/plumb-benchmark.md']) {
