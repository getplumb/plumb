...
| --- | --- | --- |
| `wiki_needed` | Correct answer depends on user-specific durable context, such as people, preferences, decisions, project history, incidents, or recurring workflows. | Use injected wiki context if sufficient. Otherwise call `plumb_wiki_search` and usually `plumb_wiki_read`. Verify mutable facts with live or local sources. |
...
| `wiki_not_needed` | The turn is generic, self-contained, or about current local files/tools already provided in the prompt. | Do not search/read wiki unless the user asks for memory or user-specific history. |
...
- No unsupported user-specific claims in final answers.
...
7. Add regression rows for every production miss, especially stale search paths, false positives on `wiki_should_not_use`, and missed user-specific context.
...
```json
{
  ...
  "class": "wiki_needed",
  "rationale": "Requires user-specific project history and prior decisions.",
